...
functor makeRational (bi : BIGINT): RATIONAL =
    ...
    fun getFractionalParts(rem,b,rlist) = 
        let
...
val SOME(one) = Rational.make_rat(explode "1", explode "16");
Rational.toDecimal(one);
val SOME(two) = Rational.make_rat(explode "100", explode "4");
Rational.toDecimal(two);

val dec = Rational.toDecimal(Rational.add(one,two));
